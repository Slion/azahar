...
class SecondaryDisplayActivity : AppCompatActivity() {
    companion object {
        ...
        private const val EXTRA_LAUNCH_BOUNDS = "launch_bounds"
         ...
         */
        fun isHosting(displayId: Int, bounds: Rect? = null): Boolean {
            if (currentDisplayId != displayId || currentBounds != bounds) return false
            if (launchPending) return true
            val instance = activeInstance ?: return false
            return !instance.isFinishing
        }

        // On dual-panel devices with a single logical display (Surface Duo 2) the host must be
        // pinned to the opposite half. setLaunchBounds() is not in the public SDK stubs, so it
        // is set reflectively.
        private fun ActivityOptions.applyLaunchBounds(bounds: Rect) {
            try {
                javaClass.getMethod("setLaunchBounds", Rect::class.java).invoke(this, bounds)
            } catch (e: Exception) {
                Log.error("SecondaryDisplayActivity cannot set launch bounds $bounds: ${e.message}")
            }
        }
        ...
        fun launch(context: Context, displayId: Int, cb: SecondaryDisplayCallback,
            ...
            try {
                ...
                Log.info("SecondaryDisplayActivity launched on display $displayId bounds=$bounds")
        ...
        fun bringToFront(context: Context) {
            ...
            val options = ActivityOptions.makeBasic().apply {
                launchDisplayId = displayId
                currentBounds?.let { applyLaunchBounds(it) }
            }
            context.startActivity(intent, options.toBundle())
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        currentBounds = intent.getParcelableExtra(EXTRA_LAUNCH_BOUNDS)
        Log.info("SecondaryDisplayActivity created for display $currentDisplayId bounds=$currentBounds")
            ...
            when (event.actionMasked) {
                MotionEvent.ACTION_DOWN, MotionEvent.ACTION_POINTER_DOWN ->
                    if (touchscreenPointerId == -1) {
                        touchscreenPointerId = event.getPointerId(event.actionIndex)
                ...
                MotionEvent.ACTION_UP, MotionEvent.ACTION_POINTER_UP, MotionEvent.ACTION_CANCEL ->
                    if (event.getPointerId(event.actionIndex) == touchscreenPointerId) {
    ...
    // A relaunch (e.g. re-hosting on the opposite half after a rotation) reaches the live
    // singleInstance activity here; sync the companion state with the new intent.
    override fun onNewIntent(intent: Intent) {
        super.onNewIntent(intent)
        setIntent(intent)
        launchPending = false
        if (!intent.hasExtra(EXTRA_LAUNCH_BOUNDS)) return
        currentDisplayId = intent.getIntExtra(EXTRA_DISPLAY_ID, currentDisplayId)
        currentBounds = intent.getParcelableExtra(EXTRA_LAUNCH_BOUNDS)
    }
